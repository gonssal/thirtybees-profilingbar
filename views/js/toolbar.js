...
String.prototype.replacePre = function () {
	return (
		this
...
$(document).ready(function() {
	...
	$toolbarContent.append('\
		<div class="sf-toolbar-block sf-toolbar-database sf-toolbar-status-normal">\
			...
			<div class="sf-toolbar-info"><div class="sf-toolbar-info-group">' + $curr.html().replaceColours().replacePre() + '</div></div>\
		</div>\
	');
	$curr.remove();
	...
	var $curr = $pC.find('.row').first();
	var $label = $curr.children('h2').children('a').text();
	$contToolbarInfo.append('<div class="sf-toolbar-info-group">' + $curr.html().replaceColours().replacePre() + '</div>');
	$curr.remove();
	// DB table stress
	var $curr = $pC.find('.row').first();
	var $label = $curr.children('h2').children('a').text();
